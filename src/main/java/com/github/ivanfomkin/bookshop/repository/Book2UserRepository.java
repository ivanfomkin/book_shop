...
public interface Book2UserRepository extends JpaRepository<Book2UserEntity, Integer> {
    ...
    @Query("SELECT count(b2u) FROM Book2UserEntity b2u JOIN UserEntity u ON u = b2u.user JOIN b2u.type b2ut WHERE u = :user AND b2ut.name = :type")
    int countBooksByUserAndStatus(UserEntity user, Book2UserType type);

    @Query("SELECT count(b2u) FROM Book2UserEntity b2u JOIN UserEntity u ON u = b2u.user JOIN b2u.type b2ut WHERE u = :user AND b2ut.name IN :types")
    ...
    @Query("SELECT b2ut.name FROM Book2UserTypeEntity b2ut JOIN Book2UserEntity b2u ON b2u.type = b2ut JOIN UserEntity u ON u = b2u.user JOIN BookEntity b ON b = b2u.book where u = :user AND b.slug = :slug")
